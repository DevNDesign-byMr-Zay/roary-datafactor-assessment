/* Aster JavaScript v026
Authenticated historical derivative: image-backend role guard plus cursor-locked expand-frame dragging.
...
*/
(function(){
  const IMAGE_BACKEND = "http://127.0.0.1:5151";
  try {
    const current = (localStorage.getItem("aster.imageToolBase") || "").trim();
    if (!current || /:5152\b/.test(current)) {
      localStorage.setItem("aster.imageToolBase", IMAGE_BACKEND);
    }
  } catch (_) {}
})();
