...
function normalizeAssetRequirements(asset, index) {
  if (!asset || typeof asset !== 'object' || Array.isArray(asset)) {
    throw new TypeError(`Asset ${index} must be an object.`);
  }
  if (asset.requires !== undefined && !Array.isArray(asset.requires)) {
    throw new TypeError(`Asset ${index} requirements must be an array.`);
  }
  return asset.requires ?? [];
}
...
export function planHolographicScene({ intent, assets = [], device }) {
  ...
  const nodes = assets.map((asset, index) => {
    const requirements = normalizeAssetRequirements(asset, index);
    return {
